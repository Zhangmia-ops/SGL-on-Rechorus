...
class LGCNEncoder(nn.Module):
    def __init__(self, user_count, item_count, emb_size, norm_adj, n_layers=3, device=None):
        ...
        self.emb_size = emb_size
        self.layers = [emb_size] * n_layers
        self.norm_adj = norm_adj  # 保存原始稀疏矩阵
        self.device = device

        self.embedding_dict = self._init_model()
        # 关键：不在__init__中创建稀疏张量
    ...
    def _ensure_sparse_tensor(self):
        """确保稀疏张量已创建并移动到正确设备"""
        if self.sparse_norm_adj is None:
            self.sparse_norm_adj = self._convert_sp_mat_to_sp_tensor(self.norm_adj)
            if self.device is not None:
                self.sparse_norm_adj = self.sparse_norm_adj.to(self.device)

    def forward(self, users, items):
        # 在forward中确保稀疏张量已创建
        self._ensure_sparse_tensor()
